    ...
    public class AccountsController : Controller
    {
        ...
        public async Task<IActionResult> Registers(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                //Update
                {
                    //Delete Old Picture And Update 
                    var OldUser = _userManager.Users.FirstOrDefault(x => x.Id == model.NewRegister.Id);
                    if (OldUser.ImageUser != null && OldUser.ImageUser != Guid.Empty.ToString())
                    {
                        var PathImage = Path.Combine(@"wwwroot/", Helper.PathSaveImageuser, OldUser.ImageUser);
		...
		public async Task<IActionResult> DeleteUser(string userId)
        {
            ...
            {
                var PathImage = Path.Combine(@"wwwroot/", Helper.PathSaveImageuser, User.ImageUser);
                if (System.IO.File.Exists(PathImage))
                    System.IO.File.Delete(PathImage);
            }
